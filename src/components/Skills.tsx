...
const skills = [
  ...
  { name: 'GraphQL', icon: '🔷', color: 'from-pink-400 to-rose-500' },
  { name: 'Redis', icon: '🔴', color: 'from-red-400 to-red-600' },
  { name: 'AWS', icon: '☁️', color: 'from-orange-400 to-yellow-500' },
  { name: 'Rust', icon: '🦀', color: 'from-orange-500 to-red-500' },
]
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {skills.map((skill, index) => (
          ...
          >
            <div className={`text-4xl mb-2 bg-gradient-to-br ${skill.color} w-fit mx-auto rounded-lg p-2 group-hover:animate-bounce`}>
              {skill.icon}
            </div>
